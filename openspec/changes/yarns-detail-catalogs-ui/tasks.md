...
- [x] 3.11 Browser verification (REGLA 4): orchestrator deletes a brand with children on `/lanas`, reads the blocked notice's counts, then deletes an empty one and confirms the tree drops it. **HECHA por el orquestador el 2026-09-20**, no por el ejecutor (que no tiene navegador). Medido: `ConfirmDialog` titula «¿Borrar pepe?», ofrece dos acciones y arranca con el foco en Cancelar; borrar una marca con 2 tipos y 0 lanas devuelve 409 y abre el aviso «No se puede borrar la marca» con una sola salida «Entendido» y el cuerpo «Esta marca todavía tiene 2 tipos. Borrá los tipos antes de eliminar la marca.» — la cláusula de lanas en cero queda omitida, y la marca sigue listada; una marca vacía desechable se borró y desapareció del panel y del árbol de filtro sin recargar, confirmado también contra `GET /api/brands`. Los controles de borrado exponen `aria-label` «Borrar <nombre>» aunque el glifo visible sea «✕».
